...
1. **多维智能筛选与「一镜三选」规约（硬约束）**：
   - **逐镜至少筛选 3 个候选配方卡**：综合分镜的叙事能量、信息形态（痛点直击/概念对比/架构扫描/终端演示/指标大板/品牌收束）与画面素材特征，**为分镜脚本中的每一个镜头筛选出至少 3 个最契合的候选动效配方卡**（涵盖 `video-shotcraft` 157 张镜头卡与 `video-talkcraft` 78 张解说卡），清晰列出各卡的**动作语法、视觉风格、呈现重点与适用场景**，呈递给用户自主选择或确认推荐方案。
   ...
   - 用户确认或选择后，形成最终的**分镜到镜头卡映射表**写入 `storyboard.md`。
...
- **默认模式：严格分步依次交互确认（Default）**：
  ...
  - **关键门禁（Gate）**：在每一个涉及选项配置的环节，**必须使用 `ask_question` 交互工具向用户呈现分析结果与推荐选项，等待用户确认/选择后方可执行该步骤的生成**，严禁一次性静默直跑！
    ...
    8. **步骤 8（短视频门禁）**：呈现分镜脚本方案供确认——黄金钩子文案（3 个可选句式）、原文字数与规划时长（每 500 字约 1 分钟，≥ 60s）、分镜数量与紧凑节奏规划、**逐镜头提供至少 3 个最契合候选镜头配方卡（`video-shotcraft` / `video-talkcraft`）供用户自主选择**、画幅（竖屏 1080×1920 / 横屏 1920×1080）、**配音音色与语速由用户自主选择（提供云扬/晓晓/云希等音色与 1.0x/1.05x 等语速选项）**；用户确认后再执行配音合成、Remotion 工程实现与渲染。
