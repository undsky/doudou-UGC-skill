...
- **调用逻辑**：
  1. **动态发现与推荐排版主题（含自定义主题）**：严禁硬编码枚举主题列表。必须先动态读取 `gzh-design` 技能的 `references/theme-index.md`（主题索引单一来源表），获取当前所有已注册主题（包括系统内置主题以及已沉淀登记的自定义主题）及其元数据（中文名、主色、适用场景、组件库路径等），依据当前文章题材与内容类型智能推荐最契合的主题样式供用户选择（全自动模式下自动选用最契合主题或默认首行）。
